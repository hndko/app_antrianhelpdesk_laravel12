        ...
        <section class="xl:col-span-4">
            <div class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm xl:sticky xl:top-24">
                ...
                <form wire:submit.prevent="saveQueue" class="space-y-5 p-5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" wire:model="user_name" placeholder="Cth: Budi" required
                                class="min-h-11 w-full rounded-lg border border-slate-300 bg-slate-50 pl-11 pr-4 py-2.5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select wire:model="technician_user_id" required
                                class="min-h-11 w-full cursor-pointer rounded-lg border border-slate-300 bg-slate-50 pl-11 pr-4 py-2.5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-bold text-slate-700">Keterangan Keluhan Perangkat (Input
                            Manual Sementara)</label>
                        <div class="relative">
                            ...
                            <textarea wire:model="description" rows="3"
                                placeholder="Tuliskan keluhan perangkat atau deskripsi masalah singkat..."
                                class="w-full resize-none rounded-lg border border-slate-300 bg-slate-50 pl-11 pr-4 py-2.5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" wire:model="duration_minutes" placeholder="Contoh: 30" required
                                    class="min-h-11 w-full rounded-lg border border-slate-300 bg-slate-50 pl-11 pr-4 py-2.5 text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-3 pt-2 sm:flex-row">
                        <button type="submit"
                            class="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-extrabold text-white shadow-lg transition hover:-translate-y-0.5
                            ...
                            <span>{{ $isEditing ? 'Simpan Perubahan' : 'Tambah Antrian' }}</span>
                        </button>
                        @if ($isEditing)
            ...
            <div class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                <div
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm font-medium text-slate-500">Data aktif yang tampil di dashboard operator.
                        </p>
                    </div>
                    <span
                        class="inline-flex w-fit items-center rounded-lg bg-emerald-50 px-3 py-1 text-xs font-black uppercase tracking-wide text-emerald-700">
                        Live Data
                    </span>
                </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="transition hover:bg-blue-50/50">
                                <td class="p-4 text-center font-mono text-lg font-black text-slate-700">{{
                                    $q->queue_number }}</td>
                                <td class="p-4 font-extrabold text-slate-900">{{ $q->user_name }}</td>
                                <td class="p-4">
                                    <div class="font-bold text-slate-800">{{ $q->laptop_id }}</div>
                                    @if ($q->description)
                                    <div class="mt-0.5 max-w-[220px] text-xs font-medium italic text-slate-500">{{
                                        Str::limit($q->description, 60) }}</div>
                                    @endif
                                    @if ($q->logs->isNotEmpty())
                                    <div class="mt-2 space-y-1 rounded-lg bg-slate-50 p-2">
                                        <p class="text-[10px] font-black uppercase tracking-wide text-slate-400">Riwayat
                                            Terakhir</p>
                                ...
                                <td class="p-4">
                                    <span
                                        class="inline-flex rounded-lg bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
                                ...
                                <td class="p-4 text-center">
                                    @if ($q->status == 'waiting')
                                    <span
                                        class="inline-flex rounded-lg bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">Menunggu</span>
                                    @elseif($q->status == 'progress')
                                    <span
                                        class="inline-flex items-center rounded-lg border border-amber-200 bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">
                                        <span class="mr-1.5 h-1.5 w-1.5 rounded-full bg-amber-500"></span>Proses
                                    </span>
                                    @else
                                    <span
                                        class="inline-flex rounded-lg border border-emerald-200 bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">Selesai</span>
                                    @endif
                                </td>
                    ...
                    <article class="p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <h3 class="mt-1 truncate text-base font-extrabold text-slate-950">{{ $q->user_name }}
                                </h3>
                                <p class="mt-0.5 text-sm font-bold text-slate-600">{{ $q->laptop_id }}</p>
                            </div>
                            @if ($q->status == 'waiting')
                            <span
                                class="shrink-0 rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-black text-slate-700">Menunggu</span>
                            @elseif($q->status == 'progress')
                            <span
                                class="shrink-0 rounded-lg border border-amber-200 bg-amber-100 px-2.5 py-1 text-[11px] font-black text-amber-700">Proses</span>
                            @else
                            <span
                                class="shrink-0 rounded-lg border border-emerald-200 bg-emerald-100 px-2.5 py-1 text-[11px] font-black text-emerald-700">Selesai</span>
                            @endif
                        </div>
                        ...
                        <div class="mt-3 space-y-1 rounded-lg bg-slate-50 p-3">
                            <p class="text-[11px] font-black uppercase tracking-wide text-slate-400">Riwayat Terakhir
                            </p>
                            @foreach ($q->logs->take(2) as $log)
                            <div class="text-xs font-semibold text-slate-500">
                                <span class="font-black text-slate-700">{{ $log->action_label }}</span>
