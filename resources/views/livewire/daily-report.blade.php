<div class="space-y-6">
                ...
                <div class="md:col-span-5">
                    ...
                    <select wire:model="selectedTechnician" id="technician" required
                        ...
                        @foreach ($technicians as $technician)
                            <option value="{{ $technician->id }}">{{ $technician->name }} ({{ $technician->personnel_status_label }})</option>
                        @endforeach
                    </select>
    ...
    @if (isset($reportData))
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
                <div>
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-black text-xl">
                    ...
                    <div class="border-t border-slate-100 pt-4 space-y-2 text-sm">
                        ...
                        <div class="flex justify-between text-slate-600">
                            ...
                            <span class="font-bold text-slate-800">{{ \Carbon\Carbon::parse($selectedDate)->translatedFormat('d F Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>

            <div class="md:col-span-2 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl shadow-lg p-6 text-white flex flex-col justify-center items-center text-center">
                ...
                <p class="text-blue-100 text-sm max-w-md">
                    Tiket layanan helpdesk yang diselesaikan pada {{ \Carbon\Carbon::parse($selectedDate)->translatedFormat('d F Y') }}.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                ...
                <span class="px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full text-xs font-bold">{{ $completedQueues->count() }} Tiket</span>
            </div>
            
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-xs font-black uppercase tracking-wide text-slate-500">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-8 text-center text-slate-400 font-medium">Tidak ada rincian tiket untuk tanggal ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @else
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
            <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-slate-500 text-sm">Pilih teknisi dan tanggal lalu klik "Tampilkan Laporan" untuk melihat rekapitulasi pekerjaan.</p>
            </div>
        </div>
    @endif
</div>
